...
%% ============================================================
%  PLANEJAMENTO QUÍNTICO - UR5e - DUAS FASES
%
%  Fase 1: q0      -> q_goal     sem carga
%  Fase 2: q_goal  -> q0         com carga de 5 kg no TCP
%
%  Modelo reduzido:
%     somente juntas 2 e 3 se movem
%
%  Entrada:
...
%  Saídas:
%     - IK reduzida
%     - trajetória quíntica ida/retorno
%     - posição, velocidade e aceleração angulares
%     - torque estimado
%     - potência mecânica
%     - energia acumulada
%     - tabela para seleção preliminar de motores
%% ============================================================
...
% Tempos de fase [s]
% O tempo total do ciclo será 2*Tf.
Tf_list = [2 5 10];
...
% Carga no retorno [kg]
m_payload_retorno = 5.0;

% Vetor gravidade no referencial da base
% z positivo para cima.
grav0 = [0; 0; -gval];

%% Parâmetros para estimativa no eixo do motor
% Nred = relação de redução.
% Se ainda não houver redutor definido, deixe Nred = 1.
Nred = [1; 1];

% Eficiência estimada da transmissão
eta = [0.75; 0.75];

% Fator de segurança para pré-seleção
FS = 1.5;

%% Opções de cálculo dinâmico
usar_dinamica_elos = true;

% Coriolis numérico é mais lento.
% Para dimensionamento preliminar, false costuma ser suficiente.
usar_coriolis = true;

%% ============================================================
%  PARÂMETROS DH E DINÂMICOS DO UR5e/UR7e
%% ============================================================
...
fprintf('Posição real atingida pela IK [m]:\n');
disp(p_final_ik);

fprintf('Erro absoluto da IK [m]: %.10f\n', erro_abs_ik);
fprintf('Erro relativo da IK [-]: %.10f\n', erro_rel_ik);
fprintf('Erro relativo da IK [%%]: %.6f %%\n', 100*erro_rel_ik);
...
if erro_abs_ik > 5e-3
    warning(['Erro final da IK maior que 5 mm. Com apenas q2 e q3 livres, ', ...
             'nem todo ponto 3D será exatamente alcançável.']);
end

%% ============================================================
%  CICLO COMPLETO: IDA SEM CARGA + RETORNO COM CARGA
%% ============================================================
...
for r = 1:length(Tf_list)
    ...
    %% Fase 1: q0 -> q_goal, sem carga
    t1_local = linspace(0, Tf, Nfase);
    [Q1, Qd1, Qdd1] = traj_quintica(q0, q_goal, t1_local, Tf);

    %% Fase 2: q_goal -> q0, com carga
    t2_local = linspace(0, Tf, Nfase);
    [Q2, Qd2, Qdd2] = traj_quintica(q_goal, q0, t2_local, Tf);

    %% Tempo global
    t1 = t1_local;
    t2 = Tf + t2_local;

    %% Concatenar removendo ponto duplicado na transição
    t = [t1, t2(2:end)];
    ...
    phase_id = [ones(1,Nfase), 2*ones(1,Nfase-1)];

    % Garante juntas travadas
    Q(fixed_idx,:) = repmat(q0(fixed_idx),1,size(Q,2));
    Qd(fixed_idx,:) = 0;
    Qdd(fixed_idx,:) = 0;
    ...
    %% Cinemática ao longo do ciclo
    p_tcp = zeros(3,Ntotal);
    Jv_all = zeros(3,6,Ntotal);
    ...
    v_tcp = zeros(3,Ntotal);
    a_tcp = zeros(3,Ntotal);
    ...
    Tau_robo = zeros(6,Ntotal);
    Tau_payload = zeros(6,Ntotal);
    Tau_total = zeros(6,Ntotal);
    P_joint = zeros(6,Ntotal);

    m_payload_t = zeros(1,Ntotal);
    ...
    for i = 1:Ntotal
        ...
        % A carga só atua na fase 2.
        if phase_id(i) == 2
            m_payload_t(i) = m_payload_retorno;
        else
            m_payload_t(i) = 0;
        end

        if m_payload_t(i) > 0
            % Força equivalente exigida para sustentar e acelerar a carga:
            % F = m*(a_tcp - gravidade)
            F_payload = m_payload_t(i)*(a_tcp(:,i) - grav0);
    ...
    tau_rms = sqrt(trapz(t, tau_active.^2, 2)/Ttotal);
    ...
    omega_motor_peak_rpm = omega_motor_peak*60/(2*pi);
    ...
    for jj = 1:length(free_idx)
        ...
        linhas_tabela(end+1,:) = {Tf, 2*Tf, junta, ...
            tau_peak(jj), tau_rms(jj), ...
            omega_peak(jj), alpha_peak(jj), ...
            P_peak(jj), E_joint(jj), ...
            tau_motor_peak(jj), tau_motor_rms(jj), ...
            omega_motor_peak(jj), omega_motor_peak_rpm(jj), ...
            P_motor_peak_est(jj)};
    end

    %% Armazenar resultados
    resultados(r).Tf = Tf;
    resultados(r).Ttotal = Ttotal;
    resultados(r).t = t;
    resultados(r).Q = Q;
    resultados(r).Qd = Qd;
    resultados(r).Qdd = Qdd;
    resultados(r).phase_id = phase_id;
    resultados(r).m_payload_t = m_payload_t;
...
ResumoMotores = cell2table(linhas_tabela, ...
    'VariableNames', {'Tf_fase_s', 'Ttotal_ciclo_s', 'Junta', ...
    ...
    'VelPico_Motor_rad_s', 'VelPico_Motor_rpm', ...
    'PotPico_Motor_comFS_W'});
...
fprintf('- Fase 1: q0 -> q_goal, sem carga no TCP.\n');
fprintf('- Fase 2: q_goal -> q0, carga de %.2f kg acoplada ao TCP.\n', m_payload_retorno);

fprintf('\nParâmetros importantes para motor:\n');
fprintf('- Torque de pico: esforço instantâneo máximo.\n');
fprintf('- Torque RMS: regime contínuo e aquecimento.\n');
fprintf('- Velocidade máxima: compatibilidade com motor/redutor.\n');
fprintf('- Aceleração máxima: severidade dinâmica da trajetória.\n');
fprintf('- Potência de pico: demanda mecânica instantânea.\n');
fprintf('- Energia acumulada: esforço mecânico global no ciclo.\n');
...
for jj = 1:length(free_idx)
    ...
    for r = 1:length(Tf_list)
        ...
        xline(resultados(r).Tf, '--', sprintf('Carga Tf=%g', resultados(r).Tf), ...
              'Color', cores(r,:), 'LabelVerticalAlignment','bottom');
    end

    xlabel('Tempo [s]');
    ylabel(sprintf('q_%d [rad]', junta));
    title(sprintf('Ciclo completo - posição angular - Junta %d', junta));
...
for jj = 1:length(free_idx)
    ...
    for r = 1:length(Tf_list)
        plot(resultados(r).t, resultados(r).Qd(junta,:), ...
             'LineWidth', 2, 'Color', cores(r,:));
        xline(resultados(r).Tf, '--', 'Color', cores(r,:));
    end

    xlabel('Tempo [s]');
    ylabel(sprintf('dq_%d [rad/s]', junta));
    title(sprintf('Ciclo completo - velocidade angular - Junta %d', junta));
    legend(compose('Tf = %g s', Tf_list), 'Location', 'best');
end
...
for jj = 1:length(free_idx)
    ...
    title(sprintf('Ciclo completo - aceleração angular - Junta %d', junta));
    legend(compose('Tf = %g s', Tf_list), 'Location', 'best');
end
...
for jj = 1:length(free_idx)
    ...
    title(sprintf('Ciclo completo - torque estimado - Junta %d', junta));
    legend(compose('Tf = %g s', Tf_list), 'Location', 'best');
end
...
for jj = 1:length(free_idx)
    ...
    title(sprintf('Ciclo completo - potência mecânica - Junta %d', junta));
    legend(compose('Tf = %g s', Tf_list), 'Location', 'best');
end
...
title('Ciclo completo - potência total demandada nas juntas ativas');
legend(compose('Tf = %g s', Tf_list), 'Location', 'best');
...
title('Ciclo completo - energia mecânica acumulada');
legend(compose('Tf = %g s', Tf_list), 'Location', 'best');

%% ============================================================
%  GRÁFICO 3D DO CICLO PARA UM TEMPO ESCOLHIDO
%% ============================================================

idx_plot = min(2, length(Tf_list));   % normalmente Tf = 5 s

t_plot = resultados(idx_plot).t;
Q_plot = resultados(idx_plot).Q;
p_plot = resultados(idx_plot).p_tcp;
phase_plot = resultados(idx_plot).phase_id;
Tf_plot = resultados(idx_plot).Tf;

% Mostra a configuração no ponto de pega q_goal
[T0_goal_plot, ~] = fkine_ur5e(q_goal, params);
...
ylabel('Y_0 [m]');
zlabel('Z_0 [m]');
title(sprintf('Ciclo completo do TCP - ida sem carga e retorno com carga - Tf = %.1f s', Tf_plot));

view(135,25);

idx_fase1 = phase_plot == 1;
idx_fase2 = phase_plot == 2;

plot3(p_plot(1,idx_fase1), p_plot(2,idx_fase1), p_plot(3,idx_fase1), ...
      'c-', 'LineWidth', 2);

plot3(p_plot(1,idx_fase2), p_plot(2,idx_fase2), p_plot(3,idx_fase2), ...
      'm--', 'LineWidth', 2);
...
text(p_des(1), p_des(2), p_des(3), ...
     '  Ponto desejado', 'FontSize', 10, 'FontWeight', 'bold');

text(p_final_ik(1), p_final_ik(2), p_final_ik(3), ...
     '  TCP na pega', 'FontSize', 10, 'FontWeight', 'bold');

% Desenhar robô em q_goal
origens = zeros(3,7);
for i = 0:6
    origens(:,i+1) = T0_goal_plot{i+1}(1:3,4);
end

plot3(origens(1,:), origens(2,:), origens(3,:), ...
      'k-o', 'LineWidth', 2, 'MarkerSize', 5);

% Referenciais em q_goal
escala_frame = 0.10;
...
for i = 1:6
    origem_junta = T0_goal_plot{i}(1:3,4);
    eixo_junta = T0_goal_plot{i}(1:3,3);
...
function [Q, Qd, Qdd] = traj_quintica(qi, qf, t_local, Tf)
...
function G = gravity_torque_required_numeric(q, params, gval)
    ...
    for i = 1:n
        ...
        % Força para sustentar o peso do elo.
