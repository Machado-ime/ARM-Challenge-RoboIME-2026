function  [tipo, pos_objeto, estado, angulo] = detector_pt(pos_garra, rgb, depth)
    ...
    if isempty(colorDetector)
        modelPath     = 'meuDetectorTreinado.mat';
        data          = load(modelPath);
...
% 
% if isempty(colorDetector)
%     modelPath     = 'C:\Users\ROBOIME5\Desktop\ARM-Challenge-RoboIME-2026\RoboCup_ARM\detector_yolov8.1.mat';
%     data          = load(modelPath);
%     colorDetector = data.detector;
%     disp('[detector_pt] Modelo carregado.');
% end
% 
% % Salvar imagem RGB temporária para o Python processar
% tempImg   = 'C:\Users\ROBOIME5\Desktop\_temp_frame.png';
% tempJson  = 'C:\Users\ROBOIME5\Desktop\_temp_result.json';
% scriptPy  = 'C:\Users\ROBOIME5\Desktop\_detectar_temp.py';
% imwrite(rgb, tempImg);
% 
% % Escrever script Python
% fid = fopen(scriptPy, 'w');
% fprintf(fid, 'from ultralytics import YOLO\nimport json\n');
% fprintf(fid, 'model = YOLO(r"%s")\n', colorDetector.modeloPt);
% fprintf(fid, 'res = model(r"%s", conf=%g)[0]\n', tempImg, colorDetector.threshold);
% fprintf(fid, 'dets=[{"classe":res.names[int(b.cls)],"score":float(b.conf),"bbox":b.xyxy[0].tolist()} for b in res.boxes]\n');
% fprintf(fid, 'open(r"%s","w").write(__import__("json").dumps(dets))\n', tempJson);
% fclose(fid);
% 
% % Executar Python
% [status, out] = system(['"' colorDetector.pythonExe '" "' scriptPy '"']);
% if status ~= 0
%     fprintf('[detector_pt] Erro Python: %s\n', out);
%     return;
% end
% 
% % Ler resultado JSON
...
% dets = jsondecode(raw);
% 
% % Converter para formato equivalente ao detect() do MATLAB
% if isempty(dets)
%     disp('Nenhum objeto detectado.');
%     return;
% end
% 
% % Montar dbox [x1 y1 x2-x1 y2-y1], dscore, dlabel
% n      = numel(dets);
% dbox   = zeros(n, 4);
% dscore = zeros(n, 1);
% dlabel = strings(n, 1);
% 
% for k = 1:n
%     b          = dets(k).bbox;           % [x1, y1, x2, y2]
%     dbox(k,:)  = [b(1), b(2), b(3)-b(1), b(4)-b(2)];
%     dscore(k)  = dets(k).score;
%     dlabel(k)  = string(dets(k).classe);
% end
% 
% % Filtrar score >= 0.6
% validIdx = dscore >= 0.6;
...
% end
    ...
    if tipo == 5 || tipo == 6
        ...
        if estado == 1
            angulo = yaw;    % em pé  → usa yaw
        else
            angulo = roll;   % deitado → usa roll
        end
        
        angulo = mod(angulo, 2*pi);     % coloca em [0, 2π)
        if angulo > pi                  % dobra para [-π, π]
            angulo = angulo - 2*pi;
        end
    ...
    switch tipo
        case 1, tipo_str = 'Can';
        case 2, tipo_str = 'Bottle';
        case 3, tipo_str = 'Spam';
        case 4, tipo_str = 'Marker';
        case 5, tipo_str = 'Cubo (Green/Purple)';
        case 6, tipo_str = 'Cubo (Blue/Red)';
        otherwise, tipo_str = 'Desconhecido';
    end
    fprintf('Tipo       : %d  (%s)\n', tipo, tipo_str);

    % Posição
    fprintf('Posição    : X = %.4f m  |  Y = %.4f m  |  Z = %.4f m\n', ...
            pos_objeto(1), pos_objeto(2), pos_objeto(3));
